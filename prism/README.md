...
│  ┌────────────────────────┴───────────────────────────────────────┐ │
│  │                       Data Services                            │ │
│  │  DefiLlama · The Graph · Snapshot · Chainlink · CryptoPanic    │ │
│  └────────────────────────────────────────────────────────────────┘ │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │                    Cache (In-Memory + TTL)                     │ │
│  └────────────────────────────────────────────────────────────────┘ │
...
The platform works fully without API keys using mock data fallbacks. For live data:
...
| CryptoPanic | Yes (free) | [cryptopanic.com/developers/api](https://cryptopanic.com/developers/api/) | News headlines for sentiment |
| The Graph | Yes (free) | [thegraph.com](https://thegraph.com/studio/) | Aave health factors, Uniswap liquidity |
...
| Sub-Score | Weight | Data Source |
|-----------|--------|-------------|
| VADER sentiment score | 40% | CryptoPanic + VADER NLP |
| Negative article ratio | 35% | CryptoPanic |
| Mention velocity spike | 25% | CryptoPanic |
...
- **Rate limits.** CryptoPanic free tier allows 200 requests/hour. The Graph and Dune have their own limits. The caching layer (15-minute TTL) mitigates this but production would need dedicated API plans.
