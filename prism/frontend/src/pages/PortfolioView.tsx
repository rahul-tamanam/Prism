...
export default function PortfolioView() {
  ...
  if (loading || !portfolio) {
    return (
      <div className="flex items-center justify-center h-64">
        <span style={{ fontFamily: 'Inter', color: 'var(--text-muted)' }}>Loading portfolio…</span>
      </div>
    )
  }
  ...
  const allocationData = portfolio.protocols
    ...
    .map(p => ({
      ...
      color: ALLOC_COLORS[p.name] || ACTION_COLORS[p.action] || 'var(--text-muted)',
    }))
  ...
  return (
    ...
    >
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {sortedProtocols.map((protocol, idx) => (
              ...
              >
                <td style={{ padding: '12px 16px', fontFamily: 'Inter', fontWeight: 400, fontSize: '0.9rem', color: 'var(--text-primary)' }}>
                  {protocol.name}
                </td>
                ...
                <td style={{ padding: '12px 16px', fontFamily: 'Inter', fontWeight: 400, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                  {protocol.worst_risk}
                </td>
                <td style={{ padding: '12px 16px', fontFamily: 'Inter', fontWeight: 400, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
        ...
        {recommendations.map((rec, idx) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              ...
              <h4 style={{ fontFamily: 'Inter', fontWeight: 700, fontSize: '0.85rem', color: 'var(--text-primary)' }}>
                {rec.title}
              </h4>
            </div>
            <p style={{ fontFamily: 'Inter', fontWeight: 400, fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
              {rec.description}
            </p>
          </motion.div>
        ))}
      ...
      <div style={{ marginTop: 32, marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
        <h2
          className="font-syne"
          style={{
            fontWeight: 700,
